...
public class AutoLavaCast extends Module {
    ...
    private final Setting<Boolean> fourWayEnabled = sgFourWay.add(new BoolSetting.Builder()
        ...
        .onChanged(val -> {
            if (!val) {
                ((Setting<Boolean>) settings.get("expanded-mode")).set(false);
                ((Setting<Boolean>) settings.get("full-connected-mode")).set(false);
            }
        })
        .build()
    );
    ...
    private final Setting<Boolean> eightWay = sgFourWay.add(new BoolSetting.Builder()
        ...
        .defaultValue(false)
        .onChanged(val -> {
            if (val) ((Setting<Boolean>) settings.get("minimal-mode")).set(true);
            else ((Setting<Boolean>) settings.get("full-connected-mode")).set(false);
        })
        .build()
    );
    ...
    private final Setting<Boolean> twelveWay = sgFourWay.add(new BoolSetting.Builder()
        ...
        .onChanged(val -> {
            if (val) {
                ((Setting<Boolean>) settings.get("minimal-mode")).set(true);
                ((Setting<Boolean>) settings.get("expanded-mode")).set(true);
            }
        })
        .build()
    );
    ...
    private void onPostTick(TickEvent.Post event) {
        ...
        if (fourWayEnabled.get() && pause) {
            ...
            if (fourWayStage > 3) {
                ChatUtils.sendMsg(Text.of("All directions and stages done!"));
                toggle();
                return;
            }
